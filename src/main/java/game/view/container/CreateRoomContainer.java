    ...
    public CreateRoomContainer(CreateRoomPanel panel){
        ...
        panel.connect(unit, state -> state, dispatcher -> new ICreateRoom() {
            ...
            @Override public void CreateRoomRequest(String roomName, String userName) {
                ...
                User user = User.newBuilder().setId(id).setName(userName).setRoomName(roomName).setIsReady(true).build();
                GrpcRoom room = GrpcRoom.newBuilder().setRoomName(roomName).setHostId(id).setHostName(userName).addUser(user).build();
